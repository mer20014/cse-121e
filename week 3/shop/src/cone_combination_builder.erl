...
cone_combinations(Top_flavors, _Bottom_flavors) when Top_flavors == nil ->
	fail;
cone_combinations(Top_flavors, Bottom_flavors) when (is_list(Top_flavors) == false) or (is_list(Bottom_flavors) == false) ->
	[];
cone_combinations(Top_flavors, Bottom_flavors) ->
	[{Top, Bottom} || Top <- Top_flavors, Bottom <- Bottom_flavors].

% TODO: Complete this function so it passes all of its unit tests
cone_combinations(Flavor_list) when Flavor_list == nil ->
	fail;
cone_combinations(Flavor_list) when is_list(Flavor_list) == false ->
	[];
cone_combinations(Flavor_list)->
	[{Top, Bottom} || Top <- Flavor_list, Bottom <- Flavor_list].

% TODO: Complete this function so it passes all of its unit tests
% 
most_popular_combinations(_Count, List) when is_list(List) == false ->
	fail;
most_popular_combinations(Count, _List) when Count == 0 ->
	[];
most_popular_combinations(Count, List) when (length(List) < Count) or (Count < 0) ->
	fail;
most_popular_combinations(Count, List)->
	{Top, _Bottom} = lists:split(Count, List),
	Top.
