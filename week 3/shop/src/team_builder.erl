...
divide(_Dividend, Divisor) when Divisor == 0->
	fail;
...
lower_divide(_Dividend, Divisor) when Divisor == 0->
	fail;
...
% division
remainder(_Dividend, Divisor) when Divisor == 0->
	fail;
remainder(Dividend, Divisor) when (is_integer(Dividend) == false) or (is_integer(Divisor) == false) ->
	fail;
